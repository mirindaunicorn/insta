...
IMAGE_TAG = latest
IMAGE_NAME = mirindaunicorn/instagram
...
install: up ## Install composer dependencies
	docker-compose exec app bash -e 'composer install'

.PHONY: seed
seed: up ## Seed database by default user admin
	docker-compose exec app bash -c 'php artisan db:seed'

.PHONY: migrate
migrate: up ## Run db migration process
	docker-compose exec app bash -c 'php artisan migrate'

.PHONY: symlink
symlink: up ## Create storage symlink
